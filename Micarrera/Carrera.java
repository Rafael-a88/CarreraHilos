...
public class Carrera {
    ...
    private void verificarEmpate() {
        List<Moto> empatados = new ArrayList<>();
        int maxDistancia = 0;

        for (Moto moto : motos) {
            if (moto.getDistanciaRecorrida() >= distanciaTotal) {
                if (moto.getDistanciaRecorrida() > maxDistancia) {
                    maxDistancia = moto.getDistanciaRecorrida();
                    empatados.clear();
                    empatados.add(moto);
                } else if (moto.getDistanciaRecorrida() == maxDistancia) {
                    empatados.add(moto);
                }
            }
        }
        ...
        if (empatados.size() > 1) {
            mensaje.append("Empate entre: ");
            for (int i = 0; i < empatados.size(); i++) {
                mensaje.append(empatados.get(i).getNombre());
                if (i < empatados.size() - 1) {
                    mensaje.append(" y ");
                }
            }
        } else if (empatados.size() == 1) {
            mensaje.append(empatados.get(0).getNombre()).append(" ha ganado!");
        }
